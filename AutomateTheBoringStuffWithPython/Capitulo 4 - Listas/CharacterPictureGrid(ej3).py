''' Ejercicio 3 - Character Picture Grid
...
of a “picture” drawn with text characters. The (0, 0) origin is in
the upper-left corner, the x-coordinates increase going right, and
the y-coordinates increase going down.
...
def dibujar(grid):
    cantidadFilas = len(grid)
    if cantidadFilas==0:
        print ('No hay dibujo fila')
        
    else:
        cantidadColumnas = len(grid[0])
        if cantidadColumnas == 0:
            print ('No hay dibujo columna')   
        
        for y in range(0,cantidadColumnas):
